...
*/
/*
This tagging criterion is a generic criterion that uses a fixed grid, but can
also deal with flux extraction in the outer region of the simulation
*/
...
class TaggingCriterion
{
  ...
  public:
    TaggingCriterion(double dx, const int a_level, const double a_L,
                     const double a_rat,
    ...
    // Fixed grid criteria. Grid levels are determined by their distance from
    // the user-defined center of the grid. Overall scaling and initial scaling
    ...
    // Extraction tagging. We want to have a fine enough resolution at the
    // extraction surface
    template <class data_t>
    ...
    // Computes each criteria, then combines them into a single criterion and
    // stores to grid
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        // Combine criteria
        data_t criterion{
            simd_max(FixedGridsTaggingCriterion, ExtractionTaggingCriterion)};

        // Write back into the flattened Chombo box. If Diagnostic is turned on,
        // then write to c_Tagging_Diagnostic
