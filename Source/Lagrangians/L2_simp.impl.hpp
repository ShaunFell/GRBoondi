...
) const
{
    ...
    data_t g_prime2 { 0 };
    m_g2_function.compute_function(g_func, g_prime, g_prime2, matter_vars, metric_vars, d1, d2);
    data_t gnn { g_prime - 2 * matter_vars.phi * matter_vars.phi * g_prime2 };
    ...
    FOR2(i,j)
    {
        total_rhs.Evec[i] += m_params.alpha2 * ( 2 * metric_vars.lapse * g_prime * gamma_UU[i][j] * matter_vars.Avec[j]  );
    }
    ...
    total_rhs.phi += m_params.alpha2 * metric_vars.lapse * g_prime * matter_vars.phi * metric_vars.K / (gnn) + advec.phi;
    FOR1(i)
    {
        total_rhs.phi += 2 * metric_vars.lapse * m_params.alpha2 * g_prime2 * matter_vars.phi * matter_vars.Avec[i] *
        ...
        FOR1(j)
        {
            total_rhs.phi +=
                gamma_UU[i][j] * (-metric_vars.lapse * m_params.alpha2 * g_prime / gnn * DA[i][j] -
                                ...
                                2 * metric_vars.lapse * m_params.alpha2 * g_prime2 / gnn * 2 * matter_vars.phi *
            ...
            FOR2(k, l)
            {
                total_rhs.phi -=
                    ...
                    (2 * metric_vars.lapse * g_prime2 / gnn * matter_vars.phi * matter_vars.Avec[i] *
                        matter_vars.Avec[j] * metric_vars.K_tensor[k][l] +
                    2 * metric_vars.lapse * g_prime2 / gnn * matter_vars.Avec[i] *
