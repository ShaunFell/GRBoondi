...
void ChargesFluxes<matter_t, background_t>::compute(
    Cell<data_t> current_cell) const
{
    ...
    */
    ...
    const MatterVars<Tensor<1, data_t>> matter_vars_d1{
        m_deriv.template diff1<MatterVars>(current_cell)};
    const MatterDiff2Vars<Tensor<2, data_t>> matter_vars_d2{
        m_deriv.template diff2<MatterDiff2Vars>(current_cell)};
    const MatterVars<data_t> advec{m_deriv.template advection<MatterVars>(
    ...
    const auto det_gamma =
        TensorAlgebra::compute_determinant_sym(metric_vars.gamma);

    // compute EM tensor
    const auto emtensor = m_matter.compute_emtensor(
        matter_vars, metric_vars, matter_vars_d1, matter_vars_d2, advec);

    /*
            compute conserved charges related to killing vectors in Kerr-Schild
       spacetime
    */

    // conserved energy
    data_t rho = -emtensor.rho * metric_vars.lapse;
    FOR1(i) { rho += metric_vars.shift[i] * emtensor.Si[i]; }
    rho *= sqrt(det_gamma);
    ...
    data_t rhoJ = 0.;
    FOR1(i) { rhoJ += emtensor.Si[i] * ddphi[i]; }
    rhoJ *= sqrt(det_gamma);
    ...
    {
        Edot +=
            metric_vars.lapse * Ni_L[i] * emtensor.rho * metric_vars.shift[i];

        FOR1(j)
        {
            Edot += -Ni_L[i] * emtensor.Si[j] *
    ...
    FOR2(i, j)
    {
        Jdot += -Ni_L[i] * metric_vars.shift[i] * emtensor.Si[j] * dxdphi[j];
        FOR1(k)
        {
    ...
    FOR4(i, j, k, l)
    {
        EM_squared +=
            ...
            (emtensor.Sij[i][j] * emtensor.Sij[k][l] -
             emtensor.Si[i] * emtensor.Si[k] * emtensor.Si[j] * emtensor.Si[l]);
    }
