...
/*
This namespace contains various functions to manipulate data, such as nearest
neighbor finders
*/
...
//  Returns a pair object, first position is the indicies of the neighbors,
//  second are the neighbors itself
template <class data_t>
std::pair<std::vector<data_t>, std::vector<std::vector<data_t>>>
find_nearest_neighbors(std::vector<std::vector<data_t>> a_matrix_data,
                       std::vector<data_t> a_query_point, int a_n_neighbors)
{
    // container for the nearest neighbors
    std::vector<std::vector<data_t>> neighbors;
    std::vector<data_t> inx_neighbors_con;

    // create vector of indicies to keep track of original location of neighbor
    // coords
    ...
    for (int inx_neighbors{0}; inx_neighbors < a_n_neighbors; inx_neighbors++)
    {
        ...
        for (int coord_vec_inx{0}; coord_vec_inx < a_matrix_data.size();
             coord_vec_inx++)
        {
            ...
            {
                dist += (coord_vec[coord] - a_query_point[coord]) *
                        (coord_vec[coord] - a_query_point[coord]);
            }
        ...
        data_indicies.erase(data_indicies.begin() +
                            min_inx); // drop corresponding index
...
template <class data_t>
data_t lin_interp_2d(std::vector<data_t> P1, std::vector<data_t> P2,
                     std::vector<data_t> P3, std::vector<data_t> query_point)
{
    // First 2 slots in the P1, P2, P3 vectors are the coordinates of the three
    // points, Last slot is the value at those points P1 = { x1, y1, value1 },
    // etc. query_points = { x, y }; return is the interpolated value
    ...
    std::vector<data_t> n = {P1P2[1] * P1P3[2] - P1P2[2] * P1P3[1],
                             P1P2[2] * P1P3[0] - P1P2[0] * P1P3[2],
                             P1P2[0] * P1P3[1] - P1P2[1] * P1P3[0]};

    // if curl is zero, then points are co-linear, and therefore cannot be
    // interpolated in 3d space
