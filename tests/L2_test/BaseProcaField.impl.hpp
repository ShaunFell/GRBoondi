...
) const
{
    ...
    double chris_sum {0.};
    FOR3(i, j, k) { chris_sum += chris_ULL[i][j][k]; };

    double DASum {0.};
    FOR2(i, j) { DASum += DA[i][j]; }
    std::cout << "my DASum: " << DASum << std::endl;
    std::cout << "my d1Asum: " << d1Asum << std::endl;
    std::cout << "my chris_sum: " << chris_sum << std::endl;
    ...
    std::cout << "my Enorm: " << Enorm << std::endl;
