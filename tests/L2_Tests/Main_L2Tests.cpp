...
int main(int argc, char *argv[])
{
    ...
    //Here we test the L2 routines
    {
        ...
        FixedBGProcaField<Potential>::Vars<Tensor<1,double>> test_matter_diff1_vars;
        FixedBGProcaField<Potential>::Diff2Vars<Tensor<2, double>> test_matter_diff2_vars;
        ...
        FOR1(i)
        {
            ...
            FOR1(j)
            {
                ...
                my_matter_diff1_vars.Avec[i][j] = test_matter_diff1_vars.Avec[i][j];
                my_matter_diff1_vars.Evec[i][j] = test_matter_diff1_vars.Evec[i][j];
                if (j>=i)
                {
                ...
                FOR1(k)
                {
                    ...
                    {
                        metric_vars.d2_shift[i][j][k] = metric_vars.d2_shift[i][k][j];
                        test_matter_diff2_vars.Avec[i][j][k] = test_matter_diff2_vars.Avec[i][k][j];
                    }

                    my_matter_diff2_vars.Avec[i][j][k] = test_matter_diff2_vars.Avec[i][j][k];
                    ...
                    FOR1(m)
                    {
                        ...
                        {
                            metric_vars.d2_gamma[i][j][k][m] = metric_vars.d2_gamma[i][j][m][k];
                        }
                    }
                }
            }
        ...
        const Tensor<3,double> chris_phys = TensorAlgebra::compute_christoffel(metric_vars.d1_gamma, gamma_UU).ULL;
        ...
        emtensor_t<double> Test_EMTest = test_proca_field.compute_emtensor(test_matter_vars, metric_vars, test_matter_diff1_vars, gamma_UU, chris_phys);
        

        //My proca field
        ProcaField::params_t myparams{proca_mass, 1.0, vec_damping};
        ProcaField my_proca_field(kerr_bh, myparams);
        emtensor_t<double> my_EMTest = my_proca_field.compute_emtensor(my_matter_vars, metric_vars, my_matter_diff1_vars, my_matter_diff2_vars, my_matter_vars);
        ...
        std::cout << "Test sum Pi: " << Test_EMTest.Si[0] + Test_EMTest.Si[1] + Test_EMTest.Si[2] << std::endl;
        std::cout << "My sum Pi:   " << my_EMTest.Si[0] + my_EMTest.Si[1] + my_EMTest.Si[2] << std::endl;
        std::cout << "Test sum Sij: " << Test_EMTest.Sij[0][0] + Test_EMTest.Sij[0][1] + Test_EMTest.Sij[0][2]  + Test_EMTest.Sij[1][0] + Test_EMTest.Sij[1][1] + Test_EMTest.Sij[1][2] + Test_EMTest.Sij[2][1] + Test_EMTest.Sij[2][2]  + Test_EMTest.Sij[2][0] << std::endl;
        std::cout << "My sum Sij:   " << my_EMTest.Sij[0][0] + my_EMTest.Sij[0][1] + my_EMTest.Sij[0][2]  + my_EMTest.Sij[1][0] + my_EMTest.Sij[1][1] + my_EMTest.Sij[1][2] + my_EMTest.Sij[2][1] + my_EMTest.Sij[2][2]  + my_EMTest.Sij[2][0] << std::endl;
        
        FixedBGProcaField<Potential>::Vars<double> test_RHS;
        ADMProcaVars::MatterVars<double> my_RHS;

        my_proca_field.matter_rhs(my_RHS, my_matter_vars, metric_vars, my_matter_diff1_vars, my_matter_diff2_vars, my_matter_vars);
        test_proca_field.matter_rhs(test_RHS, test_matter_vars, metric_vars, test_matter_diff1_vars, test_matter_diff2_vars, test_matter_vars);

        std::cout << "Test phi dot: " << test_RHS.phi << std::endl;
        std::cout << "My phi dot:   " << my_RHS.phi << std::endl;
        std::cout << "Test Avec dot sum: " << test_RHS.Avec[0] + test_RHS.Avec[1] + test_RHS.Avec[2] << std::endl;
        std::cout << "My Avec dot sum:   " << my_RHS.Avec[0] + my_RHS.Avec[1] + my_RHS.Avec[2] << std::endl;
        std::cout << "Test Evec dot sum: " << test_RHS.Evec[0] + test_RHS.Evec[1] + test_RHS.Evec[2] << std::endl;
        std::cout << "My Evec dot sum:   " << my_RHS.Evec[0] + my_RHS.Evec[1] + my_RHS.Evec[2] << std::endl;
