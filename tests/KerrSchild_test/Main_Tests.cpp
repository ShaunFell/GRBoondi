            ...
            // Excision test
            {
                ...
                for (int i{0}; i < 3 / dx; i++)
                {
                    ...
                    const Coordinates<double> coords(current_cell, dx,
                                                     kerr_params.center);
                    bool kerr_ref_excise_result{kerr_ref.excise(current_cell) <
                                                buffer};
                    ...
                    if (kerr_ref_excise_result != kerr_init_excise_result)
                    {
                        ...
                        std::cout << "Coordinate location: " << coords
                                  << std::endl;
                        std::cout << "kerr reference output: "
                                  << kerr_ref_excise_result << std::endl;
                        std::cout
                            << "kerr my output: " << kerr_init_excise_result
                            << std::endl;
                        ExcisionPass = false;
                    }
                }
