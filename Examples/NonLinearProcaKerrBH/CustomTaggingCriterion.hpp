...
class CustomTaggingCriterion
{
    ...
    public:
        CustomTaggingCriterion(double dx, int a_level, double a_L, double a_ratio, double a_threshold,
                                  std::array<double, CH_SPACEDIM> a_center, bool activate_gnn_tagging): m_dx(dx), m_L(a_L), m_init_ratio(a_ratio), m_center(a_center), m_level(a_level), m_activate_gnn_tagging(activate_gnn_tagging), m_threshold(a_threshold){};
    ...
    data_t FixedGridTagging(Cell<data_t> current_cell) const
    {
        ...
        // m_init_ratio sets the initial ratio of the 1st refinement level compared to the grid size
        // m_init_ratio = 0.25 corresponds to tagging cells around center +/ - L/4
        // Use the taxi cab metric to set the regridding ratio
    ...
    data_t gnnTagging(Cell<data_t> current_cell) const
    {
        ...
        {
            auto gnn_data = current_cell.load_vars(c_gnn); //load the gnn diagnostic data from the grid
            auto gnn_data_abs = abs(gnn_data); //compute the absolute value, taking into account SIMD functionality
            auto gnn_criterion = m_threshold * m_threshold / gnn_data_abs; //Invert the gnn value to obtain the criterion
