...
//Inherits from BaseProcaFieldLevel with background = KerrdeSitter and matter = BaseProcaField
...
public:
    ...
    virtual void initialData() override {
        ...
        ExcisionEvolution<ProcaField, KerrdeSitter> excisor(m_dx, m_p.center, 0.95, kerr_schild);
...
/*     //override method to add black hole evolution
    virtual void additionalPostTimeStep() override
    {
        ...
        if (m_time >= m_p.relaxation_time && m_p.evolve_bh &&m_p.activate_extraction && m_level == min_level && at_course_timestep_on_any_level)
        {
            ...
            // If we're in this code block, We know c_Edot and c_Jdot exist in the Diagnostic variables because of the check done in ProcaSimulationParameters.hpp
            int Edot_inx = std::find(vars_to_extract.begin(), vars_to_extract.end(), c_Edot) - vars_to_extract.begin();
            int Jdot_inx = std::find(vars_to_extract.begin(), vars_to_extract.end(), c_Jdot) - vars_to_extract.begin();
            ...
            //Note: These flux values are positive if they are outward flowing, and negative if they flow into the horizon
            ...
            m_p.background_params.spin -= a * ( Jdot_horizon / J - Edot_horizon / M );

            if (m_verbosity)
            {
            pout() << "Mass update: " << M << " -> " << m_p.background_params.mass << endl;
            pout() << "Spin update: " << a << " -> " << m_p.background_params.spin << endl;
            };

            //Check for naked singularities!
            //Note: This error check is already present in the constructor for the KerrdeSitter class.
            //          We check it here and use a different error message related to the black hole evolution code above
            if ((m_p.background_params.spin > m_p.background_params.mass) || (m_p.background_params.spin < -m_p.background_params.mass))
            {
                MayDay::Error(
                        " The dimensionless spin has exceeded physical bounds. A naked singularity has formed!!"
                );
            }
